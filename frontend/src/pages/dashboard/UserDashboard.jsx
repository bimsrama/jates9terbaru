...
import {
  Activity, Users, ShoppingBag, Home, LogOut, Settings, User, Medal, Copy, 
  ChevronRight, QrCode, Package, ChevronLeft, Clock, CheckCircle, Calendar, 
  RefreshCw, FileText, Moon, Sun, Shield, Smartphone, Check, Edit2, Camera, 
  Bot, Sparkles, MapPin, Truck, Plus, X, Send
} from 'lucide-react';
...
const UserDashboard = () => {
  ...
  const handleAddFriend = async () => { if(!friendCode) return alert("Masukkan kode teman!"); alert("Fitur add friend dalam pengembangan backend."); };
  ...
  const openCheckout = (product) => { setSelectedProduct(product); setShippingCost(0); setSelectedAddrId(null); setAppliedCoupon(null); setCouponCode(""); setShowCheckoutModal(true); };
  const handleSelectAddrCheckout = async (addrId) => {
      setSelectedAddrId(addrId);
      const addr = addresses.find(a => a.id === Number(addrId));
      if(addr) { const res = await axios.post(`${BACKEND_URL}/api/location/rate`, { city_id: addr.city_id }, { headers: getAuthHeader() }); setShippingCost(res.data.cost); setShippingMethod("JNE Regular"); }
  };
  ...
  const handleProcessPayment = async () => {
      ...
      try {
          ...
          if (response.data.success) {
              ...
              window.snap.pay(response.data.token, { onSuccess: function(result) { alert("Pembayaran Berhasil!"); fetchOrders(); }, onPending: function(result) { alert("Menunggu pembayaran!"); fetchOrders(); }, onError: function(result) { alert("Pembayaran gagal!"); } });
          }
  ...
  const ShopBanner = () => (
      <div style={{ marginBottom: '2rem', position:'relative', borderRadius: '16px', overflow:'hidden', boxShadow:'0 10px 20px rgba(0,0,0,0.1)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', background:'white', borderTop:'1px solid #eee' }}>
              ...
              <button onClick={()=>setShowOrderHistory(true)} style={{ padding:'1rem', background:'none', border:'none', cursor:'pointer', fontWeight:'bold', display:'flex', alignItems:'center', justifyContent:'center', gap:'0.5rem', color:'#059669' }}><Truck size={18}/> Status Pesanan</button>
          </div>
      </div>
  );
  ...
  return (
    <div style={{ display: 'flex', background: darkMode ? '#0f172a' : '#f8fafc', color: darkMode ? '#e2e8f0' : '#1e293b', width: '100%', height: '100vh', position: 'fixed', top: 0, left: 0, zIndex: 9999, overflow: 'hidden' }}>
      <style>{`:root { --primary: ${currentTheme.primary}; --primary-dark: ${currentTheme.text}; --theme-gradient: ${currentTheme.gradient}; --theme-light: ${currentTheme.light}; } .dark { --theme-gradient: ${currentTheme.darkGradient}; } .nav-item { display: flex; alignItems: center; gap: 0.75rem; width: 100%; padding: 0.75rem 1rem; border-radius: 8px; border: none; cursor: pointer; font-size: 0.95rem; margin-bottom: 0.25rem; text-align: left; transition: all 0.2s; color: ${darkMode ? '#94a3b8' : '#475569'}; background: transparent; } .nav-item.active { background: ${darkMode ? currentTheme.text : currentTheme.light}; color: ${darkMode ? 'white' : currentTheme.text}; font-weight: 600; } .scroll-hide::-webkit-scrollbar { display: none; } .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); display: flex; alignItems: center; justifyContent: center; z-index: 99999; } .modal-content { background: white; padding: 2rem; borderRadius: 16px; maxWidth: 500px; width: 90%; maxHeight: 90vh; overflow-y: auto; }`}</style>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, height: '100vh', overflowY: 'auto' }}>
        ...
        <main style={{ padding: isDesktop ? '2rem' : '1rem', flex: 1 }}>
          {activeTab === 'dashboard' && (
            <>
              <div style={{ marginBottom: '1.5rem', marginTop: isDesktop ? 0 : '0.5rem' }}><p className="body-medium" style={{ color: '#64748b' }}>{getGreeting()}, <strong>{overview?.user?.name}</strong>!</p></div>
                  ...
                  <Card style={{ border: 'none', borderRadius: '16px', background: 'var(--theme-gradient)', color: darkMode ? 'white' : '#1e293b', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)', overflow: 'hidden' }}>
                    <CardContent style={{ padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ position: 'relative' }}>
                          <div style={{ width: '70px', height: '70px', borderRadius: '50%', background: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, overflow: 'hidden', border: '2px solid white' }}>{overview?.user?.profile_picture ? <img src={`${BACKEND_URL}${overview.user.profile_picture}`} alt="Profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <User size={35} color={currentTheme.text} />}</div>
                      ...
                      <div>
                        ...
                        <div style={{fontSize:'0.75rem', marginTop:'0.5rem', color: darkMode?'#cbd5e1':'#475569', display:'flex', alignItems:'center', gap:'0.3rem', background:'rgba(255,255,255,0.3)', padding:'2px 8px', borderRadius:'6px', width:'fit-content', fontWeight:'bold'}}><QrCode size={12}/> Ref: {overview?.user?.referral_code}</div>
                      </div>
                    </CardContent>
                  </Card>
                    ...
                    <CardContent style={{ padding: '1.5rem' }}>
                        ...
                        <div style={{ background: darkMode ? '#334155' : '#f8fafc', borderRadius: '12px', padding: '1rem', border: darkMode ? 'none' : '1px solid #e2e8f0' }}>
                          ...
                          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem', marginBottom:'1rem'}}>
                              <div style={{background: darkMode?'#1e293b':'white', padding:'0.5rem', borderRadius:'6px', border:'1px solid #e2e8f0'}}><div style={{fontSize:'0.7rem', color:'#64748b'}}>Berhasil</div><div style={{fontSize:'1rem', fontWeight:'bold', color:'#166534'}}>{overview?.financial?.total_checkins || 0} Hari</div></div>
                              <div style={{background: darkMode?'#1e293b':'white', padding:'0.5rem', borderRadius:'6px', border:'1px solid #e2e8f0'}}><div style={{fontSize:'0.7rem', color:'#64748b'}}>Terlewat</div><div style={{fontSize:'1rem', fontWeight:'bold', color:'#991b1b'}}>{overview?.user?.missed_days || 0} Hari</div></div>
                          </div>
                  ...
                  <Card style={{ background: darkMode ? '#1e293b' : 'white', border: darkMode ? '1px solid #334155' : '1px solid #e2e8f0' }}>
                    <CardHeader style={{paddingBottom:'0.5rem'}}>
                      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <CardTitle className="heading-3" style={{display:'flex', alignItems:'center', gap:'0.5rem', fontSize: '1.1rem', color: darkMode ? 'white' : 'black'}}><Activity size={20} color={currentTheme.text}/> Misi Hari Ini</CardTitle>
                    ...
                    <CardContent>
                      ...
                      {(checkinStatus === 'completed' || checkinStatus === 'skipped') ? (<div style={{ textAlign: 'center', padding: '1.5rem', background: checkinStatus === 'completed' ? '#f0fdf4' : '#fef2f2', borderRadius: '12px' }}><h3 style={{fontWeight:'bold', color: checkinStatus === 'completed' ? '#166534' : '#991b1b'}}>Misi Selesai!</h3></div>) : (
                        <div>
                           {dailyData?.tasks?.map((task, idx) => (<div key={idx} style={{ padding: '0.8rem', background: darkMode ? '#334155' : '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom:'0.5rem', display:'flex', gap:'0.5rem', alignItems:'center', color: darkMode ? 'white' : 'black' }}><div style={{width:'8px', height:'8px', borderRadius:'50%', background: currentTheme.primary}}></div>{task}</div>))}
                           ...
                           <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem' }}><button onClick={() => handleSubmitCheckin('pending')} disabled={isSubmitting} style={{ background: '#f1f5f9', color: '#64748b', border: '1px solid #cbd5e1', padding: '0.8rem', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Nanti Saja</button><button onClick={() => handleSubmitCheckin('completed')} disabled={isSubmitting} style={{ background: currentTheme.primary, color: 'black', border: 'none', padding: '0.8rem', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Selesai</button></div>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', minWidth: 0 }}>
                  <Card ref={chatSectionRef} style={{ background: darkMode ? '#1e293b' : 'white', height: '450px', display:'flex', flexDirection:'column' }}>
                      <div style={{ padding: '1rem', borderBottom: darkMode ? '1px solid #334155' : '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '0.8rem', background: darkMode ? '#1e293b' : '#f8fafc' }}>
                        ...
                        <div><h3 style={{ fontWeight: 'bold', fontSize: '1rem', color: darkMode ? 'white' : '#0f172a', marginBottom:'2px', display:'flex', alignItems:'center', gap:'6px' }}>Dr. Alva AI <Sparkles size={16} fill={currentTheme.primary} color={currentTheme.text}/></h3><p style={{ fontSize: '0.75rem', color: darkMode ? '#94a3b8' : '#64748b' }}>Tanyakan apa saja kepada Dr. Alva</p></div>
                      </div>
                      <div style={{flex:1, overflowY:'auto', padding:'1rem'}}>
                          {chatHistory.map((msg, i) => (<div key={i} style={{ padding:'0.6rem 1rem', background: msg.role==='user' ? currentTheme.light : (darkMode?'#334155':'#f1f5f9'), borderRadius:'12px', borderBottomRightRadius: msg.role==='user' ? '2px' : '12px', borderTopLeftRadius: msg.role==='assistant' ? '2px' : '12px', marginBottom:'0.8rem', maxWidth:'85%', alignSelf: msg.role==='user' ? 'flex-end' : 'flex-start', marginLeft: msg.role==='user' ? 'auto' : '0', color: msg.role==='user' ? '#1e3a8a' : (darkMode?'#e2e8f0':'#334155'), fontSize: '0.9rem', lineHeight: '1.5' }}>{msg.content}</div>))}
          ...
          {activeTab === 'checkin' && (
              <div>
                 <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}><button onClick={() => setActiveTab('dashboard')} style={{ background: 'white', border: '1px solid #e2e8f0', padding: '0.5rem', borderRadius: '8px' }}><ChevronLeft size={20}/></button><h1 className="heading-2" style={{color: darkMode?'white':'black'}}>Riwayat Kalender</h1></div>
                 <div style={{background: darkMode ? '#1e293b' : 'white', padding:'1.5rem', borderRadius:'16px', border: darkMode?'1px solid #334155':'1px solid #e2e8f0', maxWidth:'500px', margin:'0 auto'}}>
                    ...
                    <div style={{display:'flex', gap:'1rem', marginTop:'1.5rem', justifyContent:'center'}}>
                        <div style={{display:'flex', alignItems:'center', gap:'0.3rem', fontSize:'0.8rem', color: darkMode?'#cbd5e1':'#475569'}}><div style={{width:'12px', height:'12px', background:'#dcfce7', borderRadius:'3px'}}></div> Berhasil</div>
                        <div style={{display:'flex', alignItems:'center', gap:'0.3rem', fontSize:'0.8rem', color: darkMode?'#cbd5e1':'#475569'}}><div style={{width:'12px', height:'12px', background:'#fee2e2', borderRadius:'3px'}}></div> Terlewat</div>
                    </div>
                 </div>
              </div>
          )}
          ...
          {activeTab === 'shop' && (
             <div>
                <div style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                   <div style={{display:'flex', gap:'1rem', alignItems:'center'}}><button onClick={() => setActiveTab('dashboard')} style={{ background: 'white', border: '1px solid #e2e8f0', padding: '0.5rem', borderRadius: '8px' }}><ChevronLeft size={20}/></button><h1 className="heading-2" style={{color: darkMode?'white':'black'}}>Belanja Sehat</h1></div>
                </div>
                <ShopBanner />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
                   {products.map((prod) => (
                      <Card key={prod.id} style={{ background: darkMode ? '#1e293b' : 'white', border: '1px solid #e2e8f0', overflow:'hidden', cursor:'pointer', transition:'transform 0.2s' }} onClick={() => openCheckout(prod)}>
                         <div style={{ height: '160px', background: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{prod.image_url ? <img src={`${BACKEND_URL}${prod.image_url}`} style={{width:'100%', height:'100%', objectFit:'cover'}}/> : <Package size={48} color="#cbd5e1"/>}</div>
                         <div style={{ padding: '1rem' }}><h4 style={{ fontWeight: 'bold', marginBottom: '0.3rem', color: darkMode?'white':'#0f172a' }}>{prod.name}</h4><div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'0.5rem'}}><span style={{ fontWeight: 'bold', color: '#166534' }}>Rp {prod.price.toLocaleString()}</span><div style={{background: currentTheme.primary, padding:'4px', borderRadius:'6px'}}><Plus size={16} color="white"/></div></div></div>
                      </Card>
                   ))}
                </div>
             </div>
          ...
          {activeTab === 'friends' && (
             <div style={{maxWidth:'600px', margin:'0 auto'}}>
                <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}><button onClick={() => setActiveTab('dashboard')} style={{ background: 'white', border: '1px solid #e2e8f0', padding: '0.5rem', borderRadius: '8px' }}><ChevronLeft size={20}/></button><h1 className="heading-2" style={{color: darkMode?'white':'black'}}>Teman Sehat</h1></div>
                <Card style={{marginBottom: '1.5rem', background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)', color:'white'}}>
                   <CardContent style={{padding:'2rem', textAlign:'center'}}>
                      <p style={{marginBottom:'0.5rem', opacity:0.9}}>Kode Referral Saya</p><h1 style={{fontSize:'2.5rem', fontWeight:'bold', letterSpacing:'2px', marginBottom:'1.5rem'}}>{overview?.user?.referral_code}</h1>
                      <div style={{background:'white', padding:'1rem', borderRadius:'12px', display:'inline-block', marginBottom:'1.5rem'}}><QRCodeSVG value={`https://jagatetapsehat.com/join?ref=${overview?.user?.referral_code}`} size={150} /></div>
                      <p style={{fontSize:'0.9rem', marginBottom:'1rem'}}>Ajak teman hidup sehat bersama!</p><button onClick={copyReferral} style={{background:'rgba(255,255,255,0.2)', border:'none', padding:'0.6rem 1.2rem', borderRadius:'20px', color:'white', cursor:'pointer', display:'inline-flex', alignItems:'center', gap:'0.5rem', fontWeight:'bold'}}><Copy size={16}/> Salin Kode</button>
                   </CardContent>
                </Card>
                <Card style={{marginBottom: '2rem'}}><CardContent style={{padding:'1.5rem'}}><h3 style={{fontWeight:'bold', marginBottom:'1rem', color: darkMode?'white':'black'}}>Tambah Teman</h3><div style={{display:'flex', gap:'0.5rem'}}><input placeholder="Masukkan Kode Referral Teman" value={friendCode} onChange={e=>setFriendCode(e.target.value)} style={{flex:1, padding:'0.8rem', borderRadius:'8px', border:'1px solid #ccc'}} /><button onClick={handleAddFriend} style={{background:currentTheme.primary, color:'white', border:'none', borderRadius:'8px', padding:'0 1.5rem', fontWeight:'bold', cursor:'pointer'}}>Add</button></div></CardContent></Card>
                <h3 style={{fontWeight:'bold', marginBottom:'1rem', color: darkMode?'white':'black'}}>Daftar Teman</h3>
                {myFriends.length === 0 ? <p style={{color:'#64748b', textAlign:'center'}}>Belum ada teman. Yuk ajak temanmu!</p> : (<div style={{display:'flex', flexDirection:'column', gap:'1rem'}}>{myFriends.map(f => (<div key={f.id} onClick={()=>handleShowFriendProfile(f)} style={{cursor:'pointer', padding:'1rem', display:'flex', alignItems:'center', gap:'1rem', background: darkMode?'#1e293b':'white', borderRadius:'12px', border:'1px solid #e2e8f0'}}><div style={{width:'50px', height:'50px', borderRadius:'50%', background:'#f1f5f9', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center'}}><User size={24} color="#64748b"/></div><div><h4 style={{fontWeight:'bold', color:darkMode?'white':'black'}}>{f.name}</h4><div style={{fontSize:'0.8rem', color: darkMode?'#cbd5e1':'#64748b', display:'flex', alignItems:'center', gap:'4px'}}><Medal size={12}/> {f.badge}</div></div><ChevronRight size={18} style={{marginLeft:'auto', color:'#94a3b8'}}/></div>))}</div>)}
             </div>
          )}
                    ...
                    <CardContent>
                       <div style={{display:'flex', alignItems:'center', gap:'1.5rem'}}>
                           <div style={{ position: 'relative', width:'80px', height:'80px' }}>
                              <div style={{ width: '100%', height: '100%', borderRadius: '50%', background: '#f1f5f9', overflow: 'hidden', border: '2px solid #e2e8f0' }}>{overview?.user?.profile_picture ? <img src={`${BACKEND_URL}${overview.user.profile_picture}`} alt="Profile" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center'}}><User size={40} color="#94a3b8"/></div>}</div>
                       ...
                       <div style={{display:'flex', gap:'1rem', flexWrap:'wrap'}}>
                          {Object.values(THEMES).map((theme) => (
                             <div key={theme.id} onClick={() => changeThemeColor(theme.id)} style={{ cursor: 'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.3rem' }}>
                                <div style={{ width:'40px', height:'40px', borderRadius:'50%', background: theme.gradient, border: themeColor === theme.id ? `3px solid ${darkMode?'white':'#1e293b'}` : '1px solid #ccc', boxShadow: '0 2px 5px rgba(0,0,0,0.1)' }}>{themeColor === theme.id && <div style={{width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center'}}><Check size={20} color="white" style={{dropShadow:'0 1px 2px rgba(0,0,0,0.5)'}}/></div>}</div>
                  ...
                  <Card style={{ background: darkMode ? '#1e293b' : 'white', border: darkMode ? '1px solid #334155' : '1px solid #e2e8f0' }}>
                    ...
                    <CardContent>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <div><label style={{ fontSize: '0.85rem', color: '#64748b' }}>Nomor WhatsApp</label><input type="text" value={overview?.user?.phone || ""} disabled style={{ width:'100%', padding: '0.8rem', background: darkMode ? '#334155' : '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '6px', color: darkMode?'white':'black' }} /></div>
                        <div><label style={{ fontSize: '0.85rem', color: '#64748b' }}>Kode Referral Saya</label><div style={{ display:'flex', gap:'0.5rem' }}><input type="text" value={overview?.user?.referral_code || ""} disabled style={{ flex:1, padding: '0.8rem', background: darkMode ? '#334155' : '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '6px', fontWeight:'bold', letterSpacing:'1px', color: darkMode?'white':'black' }} /><button onClick={copyReferral} style={{ background: currentTheme.primary, color:'black', border:'none', borderRadius:'6px', padding:'0 1rem', cursor:'pointer' }}><Copy size={18}/></button></div></div>
                      </div>
                    </CardContent>
                  </Card>
                  <Card style={{ background: darkMode ? '#1e293b' : 'white', border: darkMode ? '1px solid #334155' : '1px solid #e2e8f0' }}>
                    ...
                    <CardContent>
                       <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
                          <button onClick={toggleDarkMode} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', width:'100%', padding:'1rem', background: darkMode ? '#334155' : '#f8fafc', border:'1px solid #cbd5e1', borderRadius:'8px', cursor:'pointer', color: darkMode?'white':'black' }}>
                             ...
                             <div style={{ width:'40px', height:'20px', background: darkMode ? currentTheme.primary : '#cbd5e1', borderRadius:'20px', position:'relative', transition:'background 0.3s' }}><div style={{ width:'16px', height:'16px', background:'white', borderRadius:'50%', position:'absolute', top:'2px', left: darkMode ? '22px' : '2px', transition:'left 0.3s' }}></div></div>
                          </button>
      ...
      {showAddressModal && (
          <div className="modal-overlay">
             <div className="modal-content">
                 <div style={{display:'flex', justifyContent:'space-between', marginBottom:'1.5rem'}}><h3 style={{fontWeight:'bold', fontSize:'1.2rem'}}>Alamat Saya</h3><button onClick={()=>setShowAddressModal(false)}><X size={24}/></button></div>
                 ...
                 <div style={{borderTop:'1px solid #e2e8f0', paddingTop:'1.5rem'}}>
                     ...
                     <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0.5rem'}}><input placeholder="Nama Penerima" value={newAddr.name} onChange={e=>setNewAddr({...newAddr, name:e.target.value})} style={{width:'100%', padding:'0.7rem', marginBottom:'0.5rem', border:'1px solid #ccc', borderRadius:'6px'}}/><input placeholder="No HP" value={newAddr.phone} onChange={e=>setNewAddr({...newAddr, phone:e.target.value})} style={{width:'100%', padding:'0.7rem', marginBottom:'0.5rem', border:'1px solid #ccc', borderRadius:'6px'}}/></div>
                     ...
                     {newAddr.prov_id && (<select value={newAddr.city_id} onChange={handleCityChange} style={{width:'100%', padding:'0.7rem', marginBottom:'0.5rem', border:'1px solid #ccc', borderRadius:'6px'}}><option value="">Pilih Kota/Kabupaten</option>{cities.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>)}
      ...
      {showCheckoutModal && selectedProduct && (
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <div style={{marginBottom:'1.5rem', background:'#f8fafc', padding:'1rem', borderRadius:'8px', display:'flex', gap:'1rem', alignItems:'center'}}>{selectedProduct.image_url ? <img src={`${BACKEND_URL}${selectedProduct.image_url}`} style={{width:'60px', height:'60px', borderRadius:'8px', objectFit:'cover'}}/> : <Package size={40}/>}<div><h4 style={{fontWeight:'bold'}}>{selectedProduct.name}</h4><p style={{color:'#166534', fontWeight:'bold'}}>Rp {selectedProduct.price.toLocaleString()}</p></div></div>
                <div style={{marginBottom:'1.5rem'}}>
                    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'0.5rem'}}><label style={{fontWeight:'bold', fontSize:'0.9rem'}}>Alamat Pengiriman</label><button onClick={()=>{fetchAddresses(); setShowAddressModal(true)}} style={{fontSize:'0.8rem', color: currentTheme.primary, background:'none', border:'none', cursor:'pointer'}}>+ Tambah Alamat</button></div>
                    {addresses.length > 0 ? (<select onChange={(e)=>handleSelectAddrCheckout(e.target.value)} style={{width:'100%', padding:'0.5rem', borderRadius:'6px', border:'1px solid #ccc'}}><option value="">Pilih Alamat Tersimpan</option>{addresses.map(a => <option key={a.id} value={a.id}>{a.label} - {a.city_name}</option>)}</select>) : (<p style={{color:'#64748b', fontSize:'0.9rem'}}>Belum ada alamat. Silakan tambah alamat.</p>)}
                </div>
                <div style={{borderTop:'1px solid #eee', paddingTop:'1rem', marginBottom:'1.5rem'}}><div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.3rem'}}><span>Subtotal</span><span>Rp {selectedProduct.price.toLocaleString()}</span></div><div style={{display:'flex', justifyContent:'space-between', marginBottom:'0.3rem'}}><span>Ongkos Kirim (JNE)</span><span>Rp {shippingCost.toLocaleString()}</span></div><div style={{display:'flex', justifyContent:'space-between', fontWeight:'bold', fontSize:'1.2rem', marginTop:'0.5rem'}}><span>Total</span><span>Rp {(selectedProduct.price + shippingCost).toLocaleString()}</span></div></div>
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem'}}><button onClick={()=>setShowCheckoutModal(false)} style={{padding:'0.8rem', border:'1px solid #ccc', background:'white', borderRadius:'8px', fontWeight:'bold', cursor:'pointer'}}>Batal</button><button onClick={handleProcessPayment} disabled={!selectedAddrId} style={{padding:'0.8rem', border:'none', background: selectedAddrId ? '#ee4d2d' : '#cbd5e1', color:'white', borderRadius:'8px', fontWeight:'bold', cursor: selectedAddrId ? 'pointer' : 'not-allowed'}}>Bayar Sekarang</button></div>
            </div>
        </div>
      )}
      ...
      {showOrderHistory && (
          <div className="modal-overlay">
            <div className="modal-content">
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem'}}><h3 style={{ fontSize:'1.4rem', fontWeight:'bold' }}>Status Pesanan</h3><button onClick={()=>setShowOrderHistory(false)} style={{background:'none', border:'none', cursor:'pointer'}}><X size={24}/></button></div>
                {myOrders.length === 0 ? <p style={{color:'#64748b', textAlign:'center'}}>Belum ada pesanan.</p> : (<div style={{display:'flex', flexDirection:'column', gap:'1rem'}}>{myOrders.map((order, idx) => (<div key={idx} style={{border:'1px solid #e2e8f0', borderRadius:'12px', padding:'1rem', marginBottom:'1rem'}}><div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}><div><h4 style={{fontWeight:'bold'}}>{order.product_name}</h4><p style={{fontSize:'0.8rem', color:'#64748b'}}>Resi: {order.resi || '-'}</p></div><span style={{fontSize:'0.75rem', fontWeight:'bold', padding:'2px 8px', borderRadius:'12px', background: '#dcfce7', color: '#166534'}}>{order.status}</span></div><div style={{marginTop:'1rem', display:'flex', justifyContent:'space-between', alignItems:'center'}}><div style={{fontSize:'0.8rem', color:'#64748b'}}>{order.date}</div>{(order.status === 'pending' || order.status === 'paid') && (<button onClick={()=>handleCancelOrder(order.order_id)} style={{background:'#fee2e2', color:'#991b1b', border:'none', padding:'0.4rem 0.8rem', borderRadius:'6px', fontSize:'0.8rem', cursor:'pointer'}}>Batalkan</button>)}</div></div>))}</div>)}
            </div>
          </div>
      )}
      ...
      {showPrivacyModal && (
         <div className="modal-overlay">
            <div style={{ background: darkMode ? '#1e293b' : 'white', color: darkMode ? 'white' : 'black', padding: '2rem', borderRadius: '16px', maxWidth: '500px', width: '90%', maxHeight:'80vh', overflowY:'auto' }}>
               ...
               <div style={{ fontSize:'0.9rem', lineHeight:'1.6', marginBottom:'1.5rem', color: darkMode ? '#cbd5e1' : '#334155' }}><p><strong>1. Pengumpulan Data:</strong> Kami mengumpulkan data nama, nomor WhatsApp, dan log aktivitas kesehatan Anda untuk keperluan monitoring program Vitalyst.</p><p><strong>2. Penggunaan Data:</strong> Data Anda digunakan untuk memberikan rekomendasi kesehatan yang personal oleh AI dan tim ahli kami.</p><p><strong>3. Keamanan:</strong> Kami tidak membagikan data pribadi Anda kepada pihak ketiga tanpa izin, kecuali untuk keperluan pengiriman produk (ekspedisi).</p><p><strong>4. Hak Pengguna:</strong> Anda berhak meminta penghapusan akun sewaktu-waktu melalui Admin.</p></div>
      ...
      {showQRModal && (
        <div className="modal-overlay" onClick={() => setShowQRModal(false)}>
            <div style={{ background: 'white', padding: '2rem', borderRadius: '16px', textAlign: 'center', maxWidth: '350px', width: '90%' }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ marginBottom: '1.5rem' }}><div style={{ background: 'white', padding: '1rem', border: '1px solid #e2e8f0', borderRadius: '12px', display: 'inline-block', marginBottom: '1rem' }}><QRCodeSVG value={`https://jagatetapsehat.com/friend/${overview?.user?.referral_code}`} size={160} /></div></div>
        ...
        <div className="modal-overlay" onClick={() => setShowFriendProfile(false)}>
            <div style={{ background: 'white', borderRadius: '16px', maxWidth: '350px', width: '90%', overflow: 'hidden', position: 'relative' }} onClick={e => e.stopPropagation()}>
                <div style={{ background: `linear-gradient(135deg, ${currentTheme.light} 0%, ${currentTheme.primary} 100%)`, padding: '2rem 1rem', textAlign: 'center' }}>
                    <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'white', margin: '0 auto 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', overflow: 'hidden', border: '2px solid white' }}>{friendData.profile_picture ? <img src={`${BACKEND_URL}${friendData.profile_picture}`} alt={friendData.name} style={{width:'100%', height:'100%', objectFit:'cover'}} /> : <User size={40} color={currentTheme.text} />}</div>
                ...
                <div style={{ padding: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                        <div style={{ textAlign: 'center', padding: '0.8rem', background: '#f0fdf4', borderRadius: '8px' }}><div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#166534' }}>{friendData.total_checkins}</div><div style={{ fontSize: '0.7rem', color: '#166534' }}>Check-in</div></div>
                        <div style={{ textAlign: 'center', padding: '0.8rem', background: '#fff7ed', borderRadius: '8px' }}><div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#ea580c' }}>{friendData.challenge_day}</div><div style={{ fontSize: '0.7rem', color: '#ea580c' }}>Hari Jalan</div></div>
                    </div>
