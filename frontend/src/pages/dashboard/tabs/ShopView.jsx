...
const ShopView = ({ BACKEND_URL, getAuthHeader, darkMode, currentTheme, setActiveTab }) => {
    ...
    const [shippingMethod, setShippingMethod] = useState("jne"); // jne atau pickup
    ...
    const fetchAddresses = async () => {
        try { const res = await axios.get(`${BACKEND_URL}/api/user/address`, { headers: getAuthHeader() }); setAddresses(res.data); } catch (e) {}
    };
    const fetchOrders = async () => {
        try { const res = await axios.get(`${BACKEND_URL}/api/user/orders`, { headers: getAuthHeader() }); setMyOrders(res.data); } catch (e) {}
    };
    ...
    const handleSelectAddrCheckout = async (addrId) => {
        ...
        if (shippingMethod === 'pickup') { setShippingCost(0); return; }
                ...
                <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999}}>
                    <div style={{background:'white', padding:'2rem', borderRadius:'16px', width:'90%', maxWidth:'400px', maxHeight:'90vh', overflowY:'auto', color:'black'}}>
                        ...
                        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem'}}>
                            ...
                            <button onClick={handleProcessPayment} style={{padding:'0.8rem', border:'none', background: (shippingMethod==='jne' && !selectedAddrId) ? '#ccc' : currentTheme.primary, color:'white', borderRadius:'8px', fontWeight:'bold', cursor: 'pointer'}}>Bayar</button>
                        </div>
                    </div>
                </div>
            )}
