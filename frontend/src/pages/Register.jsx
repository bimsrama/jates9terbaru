...
import { 
  ...
  RefreshCcw, CheckCircle, ShieldCheck, Phone, User, Lock, Edit2, RefreshCw 
} from 'lucide-react';
...
const Register = () => {
  ...
  const { register, getAuthHeader } = useAuth(); // register() ini nanti dipakai di tahap akhir (Step 5 logic)
  ...
  const handleInitialSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Minta OTP ke Backend
      await axios.post(`${BACKEND_URL}/api/auth/request-otp`, { phone_number: formData.phone_number });
      setShowOtpModal(true);
      startResendTimer();
    } catch (err) {
      setError(err.response?.data?.message || 'Gagal kirim OTP. Cek nomor WA Anda.');
    } finally {
      setLoading(false);
    }
  };

  // --- LOGIC OTP: VERIFY & CREATE ACCOUNT ---
  const handleVerifyAndRegister = async () => {
    ...
    try {
      ...
      // 2. Register Akun (Simpan ke DB)
      const regRes = await register({ // Fungsi dari AuthContext
        name: formData.name,
        phone_number: formData.phone_number,
      ...
      if (regRes.success) {
        ...
        await fetchChallenges(); // Ambil data challenge
        setStep(2); // Lanjut ke Step 2
      } else {
        alert(regRes.error || "Gagal membuat akun.");
      }

    } catch (err) {
      alert(err.response?.data?.message || "Kode OTP Salah.");
    } finally {
      setOtpLoading(false);
    }
  };
  ...
  const fetchChallenges = async () => {
    ...
    } catch (err) { console.error("Gagal load challenge", err); }
  };
  ...
  const handleSelectChallenge = async (id) => {
    ...
    try {
      ...
      if (qRes.data && qRes.data.length > 0) {
        ...
        setStep(3); // Masuk Step 3 (Quiz)
      } else {
        setStep(5); // Skip Quiz jika tidak ada soal
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--gradient-hero)', paddingTop: '2rem', paddingBottom: '2rem' }}>
      ...
      <Card style={{ maxWidth: '500px', width: '100%', background: 'var(--bg-card)', border: '1px solid var(--border-light)', display: 'flex', flexDirection: 'column', minHeight: '650px', position: 'relative' }}>
        ...
        <CardContent style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
          ...
          {error && (
            <div style={{ background: '#fee2e2', border: '1px solid #fecaca', color: '#dc2626', padding: '0.75rem', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.875rem', textAlign: 'center' }}>
              {error}
            </div>
          )}
          ...
          {step === 1 && (
            <form onSubmit={handleInitialSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div style={{ position: 'relative' }}>
                ...
                <input type="text" name="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Nama Lengkap" required style={inputStyle} />
              </div>
              <div style={{ position: 'relative' }}>
                <Phone size={20} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                <input type="tel" name="phone_number" value={formData.phone_number} onChange={(e) => setFormData({ ...formData, phone_number: e.target.value })} placeholder="Nomor WhatsApp" required style={inputStyle} />
              </div>
              <div style={{ position: 'relative' }}>
                <Lock size={20} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                <input type="password" name="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="Password" required minLength={6} style={inputStyle} />
              </div>
              <div style={{ position: 'relative' }}>
                <Lock size={20} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                <input type="password" name="confirm_password" value={formData.confirm_password} onChange={(e) => setFormData({ ...formData, confirm_password: e.target.value })} placeholder="Ulangi Password" required style={inputStyle} />
              </div>
              ...
              <Button type="submit" className="btn-primary" disabled={loading} style={{ width: '100%', marginTop: '0.5rem' }}>
                {loading ? <Loader2 className="animate-spin" /> : <>Daftar & Verifikasi WA <ArrowRight size={18} style={{ marginLeft: '0.5rem' }} /></>}
              </Button>
              
              <div style={{ textAlign: 'center', marginTop: '1rem' }}>
                <p className="body-small" style={{ color: 'var(--text-secondary)' }}>Sudah punya akun? <Link to="/login" style={{ color: 'var(--accent-text)', fontWeight: 600 }}>Masuk</Link></p>
              </div>
            </form>
          )}

          {/* === STEP 2: PILIH CHALLENGE === */}
          {step === 2 && (
          ...
          {step === 4 && quizResult && (
            <div style={{ textAlign: 'center' }}>
              ...
              <Button variant="outline" onClick={() => { setStep(2); setAnswers({}); setQuizResult(null); }} style={{ width: '100%', marginTop: '1rem', border: '1px solid var(--border-light)', color: 'var(--text-secondary)' }}>
          ...
          {step === 5 && (
            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
              ...
              <p className="body-medium" style={{ color: '#4b5563', marginBottom: '2rem' }}>Registrasi selesai. Mengalihkan ke Dashboard...</p>
